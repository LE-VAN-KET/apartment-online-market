...
//    void cancelOrderExpired(@Param("status")OrderStatus status, @Param("timeNow") Timestamp timeNow);
    ...
    @Query("select new com.cdcn.apartmentonlinemarket.orders.model.OrderHistoryResponse(o.id, o.reference, SUM(oit.price * oit.quantity), o.orderStatus, oi.customerName, oi.phone, oi.email, oi.roomId) FROM Orders o LEFT JOIN OrderInfo oi ON o.id = oi.orderId LEFT JOIN OrderItem oit ON o.id = oit.orderId WHERE o.user.id = :user_id GROUP BY o.id, o.reference, o.orderStatus, oi.customerName, oi.phone, oi.email, oi.roomId ORDER BY o.createAt DESC")
    List<OrderHistoryResponse> orderHistories(@Param("user_id") UUID user_id);

    @Query("select new com.cdcn.apartmentonlinemarket.orders.model.OrderDetailResponse(o.id, p.type, p.totalAmount, p.details, o.reference, p.currentCode, p.status,o.orderStatus, p.createdDate, oi.customerName, oi.roomId, oi.phone, oi.email) FROM Orders o LEFT JOIN OrderInfo oi ON o.id = oi.orderId LEFT JOIN Payment p ON p.orderId = o.id LEFT JOIN OrderItem oit ON o.id = oit.orderId WHERE o.id = :order_id GROUP BY o.id, p.type, p.totalAmount, p.details, o.reference, p.currentCode, p.status,o.orderStatus, p.createdDate, oi.customerName, oi.roomId, oi.phone, oi.email ORDER BY  o.createAt DESC")
    ...
    @Query("select new com.cdcn.apartmentonlinemarket.orders.model.OrderHistoryResponse(o.id, o.reference, SUM(oit.price * oit.quantity), o.orderStatus, oi.customerName, oi.phone, oi.email, oi.roomId) FROM Orders o LEFT JOIN OrderInfo oi ON o.id = oi.orderId LEFT JOIN OrderItem oit ON o.id = oit.orderId LEFT JOIN Product p on oit.productId = p.id LEFT JOIN Store s ON s.id = p.store.id WHERE s.id = :store_id GROUP BY o.id, o.reference, o.orderStatus, oi.customerName, oi.phone, oi.email, oi.roomId ORDER BY o.createAt DESC")
